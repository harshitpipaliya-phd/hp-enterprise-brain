...
final class IntelligenceOverviewController extends Controller
{
    ...
    public function enterpriseOverview(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'trends' => [
                'healthScore' => $this->trendSeries($tenant, 'score.decisionAcceptance', 180),
                'risk' => $this->trendSeries($tenant, 'score.riskCoverage', 180),
                'capability' => $this->trendSeries($tenant, 'capability.coverage', 180),
                'decision' => $this->trendSeries($tenant, 'decision.latencyMedianHours', 180),
                'execution' => $this->trendSeries($tenant, 'recommendations.pending', 180),
                'outcome' => $this->trendSeries($tenant, 'score.recommendationAccuracy', 180),
            ],
        ]);
    }
