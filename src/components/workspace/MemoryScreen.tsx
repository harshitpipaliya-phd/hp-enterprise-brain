...
export default function MemoryScreen({ tenantId }: { tenantId: string }) {
  ...
  const renderReuseRate = (rate: number | null) => {
    if (rate === null) {
      return <span style={{ color: theme.textMuted }}>not yet measurable</span>;
    }
    ...
    <div style={{ fontFamily: 'system-ui, sans-serif', maxWidth: 1200, margin: '0 auto', padding: 24, backgroundColor: theme.bg, color: theme.text, minHeight: '100vh' }}>
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 24 }}>
          <div style={{ padding: 14, borderRadius: 8, border: `1px solid ${theme.border}`, backgroundColor: theme.surface }}>
            <div style={{ fontSize: 11, color: theme.textMuted, textTransform: 'uppercase' }}>Learnings Written</div>
            <div style={{ fontSize: 24, fontWeight: 600 }}>{stats.learningsWritten}</div>
          </div>
          <div style={{ padding: 14, borderRadius: 8, border: `1px solid ${theme.border}`, backgroundColor: theme.surface }}>
            <div style={{ fontSize: 11, color: theme.textMuted, textTransform: 'uppercase' }}>Reusable</div>
            <div style={{ fontSize: 24, fontWeight: 600 }}>{stats.learningsReusable}</div>
          </div>
          <div style={{ padding: 14, borderRadius: 8, border: `1px solid ${theme.border}`, backgroundColor: theme.surface }}>
            <div style={{ fontSize: 11, color: theme.textMuted, textTransform: 'uppercase' }}>Reuse Rate</div>
            <div style={{ fontSize: 24, fontWeight: 600 }}>{renderReuseRate(stats.reuseRate)}</div>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <p style={{ color: theme.textMuted }}>No learnings yet. The loop must run, record an outcome, and the outbox consumer must write a learning before memory has anything to show.</p>
      ) : (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                      <strong>{l.pattern}</strong>
                      {l.reusable ? (
                        <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, backgroundColor: '#10b98120', color: '#10b981' }}>Reusable</span>
                      ) : (
                        <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, backgroundColor: '#6b728020', color: theme.textMuted }}>Not reusable</span>
                      )}
                    </div>
                    {l.description && (
                      <p style={{ fontSize: 13, color: theme.textMuted, margin: '4px 0' }}>{l.description}</p>
                    )}
                ...
                {isSelected && (
                  <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${theme.border}` }}>
                    ...
                    {grounded.length === 0 ? (
                      <p style={{ fontSize: 12, color: theme.textMuted }}>Not yet grounded on any later entity.</p>
                    ) : (
                      <div style={{ display: 'grid', gap: 6 }}>
                        {grounded.map((g) => {
                          const payload = JSON.parse(g.payload || '{}');
                          return (
                            <div key={g.id} style={{ fontSize: 12, padding: 8, borderRadius: 6, backgroundColor: theme.bg, border: `1px solid ${theme.border}` }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <span>{payload.groundedEntityType || 'Unknown'}: {g.correlation_id?.slice(0, 8)}...</span>
                                <span style={{ color: theme.textMuted }}>{g.created_at?.slice(0, 10)}</span>
                              </div>
                            </div>
                          );
                        })}
