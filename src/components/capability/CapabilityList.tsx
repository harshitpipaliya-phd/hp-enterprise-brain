...
export default function CapabilityList({
  ...
  const visible = useMemo(
    () =>
      (results ?? capabilities).filter(
        (cap) =>
          (type === 'all' || cap.capabilityType === type) &&
          (criticality === 'all' || cap.criticality === criticality),
      ),
    [results, capabilities, type, criticality],
  );
  ...
  const categories = new Set(capabilities.map((cap) => cap.category).filter(Boolean)).size;

  return (
    <div className="cap-library">
      <header className="cap-library__hero">
        <div>
          <span>Capability intelligence</span>
          <h2>Build the capabilities your organization relies on</h2>
          <p>Define, prioritize, and assign the skills, knowledge, and behaviours that power effective execution.</p>
        </div>
        <div className="cap-library__hero-mark">
          <Target size={34} />
      ...
      <section className="cap-library__kpis">
        <Kpi icon={<Boxes />} label="Total capabilities" value={capabilities.length} hint="In this organization" />
        <Kpi icon={<CircleGauge />} label="Active" value={active} hint={`${Math.max(0, capabilities.length - active)} inactive or archived`} />
        <Kpi icon={<ShieldAlert />} label="High criticality" value={critical} hint="High or critical priority" />
        <Kpi icon={<Tag />} label="Categories" value={categories} hint="Capability domains" />
      </section>
            ...
            {visible.map((cap) => (
              <article key={cap.id} className="cap-card">
                ...
                <div className="cap-card__meta">
                  ...
                  <span>{cap.capabilityType || 'competency'}</span>
                  <span>{cap.difficulty || 'intermediate'}</span>
                </div>
                <footer>
                  <span>Version {cap.version || 1}</span>
